...
Merge is one way to handle this divergence. It adds a new commit which has two parents and updates one of the branch references (master).
![git-merged-divergence.png]({{site.baseurl}}/archives/images/git-merged-divergence.png)

Another way to handle this same situation is to use rebase. It adds a new chain of commits (only one in this example) to one side. Those new commit replay the changes on the other side of the divergence since the common ancestor. Then it updates the other branch reference (experiment).
