...
Strangely, Marx insists that the only way to measure value is by comparing commodities. Only a few paragraphs later, he goes back to labour being the source of value, albeit not being value itself (labour only becomes value when it is embodied into a commodity). (I guess services are not "value" then, yet another hole).
