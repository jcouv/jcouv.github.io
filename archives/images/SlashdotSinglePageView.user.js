...
// update (2005/07/26): 
//  - fixed problem with screen scraping the base domain pages ("slashdot.org")
//  - added "(Show/hide)" hints to links
...
function instrumentLink(link) {
    link.insertBefore(document.createTextNode("(Show/hide) "), link.firstChild);

    var inject = function(body) { 
        var div = document.createElement("div");
        div.style.border = "2px solid";
        var comments = extractComments(body);
...
// note: for some reason, the screen scraping doesn't work on the "slashdot.org" domain
// on articles.pl, if domain is "slashdot.org", redirect to "apple.slashdot.org"
var matches = document.location.href.match(/^http:\/\/slashdot.org\/(article\.pl.*)$/);
if (matches != null) 
{
    document.location.href = "http://apple.slashdot.org/" + matches[1];
    return;
}
